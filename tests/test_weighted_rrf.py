...
def test_equal_weights_preserve_standard_rrf_ordering_scale():
    channel_ranks = {"lexical": 2, "dense": 1}
    weighted_score = weighted_reciprocal_rank_fusion_score(
        channel_ranks,
        {"lexical": 0.5, "dense": 0.5},
    )

    assert weighted_score == pytest.approx(
        reciprocal_rank_fusion_score(channel_ranks) / 2.0
    )


def test_weighted_rrf_score_accepts_zero_weight_channel():
    score = weighted_reciprocal_rank_fusion_score(
        {"lexical": 1, "dense": 2},
        {"lexical": 0.0, "dense": 1.0},
    )

    assert score == pytest.approx(1.0 / 62.0)
